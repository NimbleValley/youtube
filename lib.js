...
function YouTubeToHtml5() {
    ...
    this.options = {},
    this.defaultOptions)
        this.options[b] = b in a ? a[b] : this.defaultOptions[b];
...
YouTubeToHtml5.prototype.getHooks = function(a, b) {
    var c = [];
...
YouTubeToHtml5.prototype.addHook = function(a, b) {
    a in this.globalHooks || (this.globalHooks[a] = []),
    a in this.hooks || (this.hooks[a] = []),
...
YouTubeToHtml5.prototype.doAction = function(a) {
    for (var b = this.getHooks("actions", a), c = arguments.length, d = Array(1 < c ? c - 1 : 0), e = 1; e < c; e++)
        d[e - 1] = arguments[e];
...
YouTubeToHtml5.prototype.applyFilters = function(a, b) {
    for (var c = this.getHooks("filters", a), d = arguments.length, e = Array(2 < d ? d - 2 : 0), f = 2; f < d; f++)
        e[f - 2] = arguments[f];
...
YouTubeToHtml5.prototype.parseYoutubeMeta = function(a) {
    ...
    var e = a.data || {};
    return e = this.applyFilters("api.response", e, a),
    e.hasOwnProperty("url_encoded_fmt_stream_map") && (c = c.concat(e.url_encoded_fmt_stream_map.split(",").map(function(a) {
    ...
    c.forEach(function(a) {
        if (a && "itag"in a && b.itagMap[a.itag]) {
            ...
            if ("url"in a && a.url ? c.url = a.url : "signatureCipher"in a,
            "audioQuality"in a && a.audioQuality && (c.hasAudio = !0),
            c.label = "qualityLabel"in a && a.qualityLabel ? a.qualityLabel : b.itagMap[a.itag],
            "mimeType"in a) {
                var e = a.mimeType.match(/^(audio|video)(?:\/([^;]+);)?/i);
                e[1] && (c.type = e[1]),
                e[2] && (c.mime = e[2]),
                c.browserSupport = b.canPlayType("".concat(c.type, "/").concat(c.mime))
            }
            c.url && d.push(c)
        }
    }),
    d = this.applyFilters("api.results", d, e),
    d
}
,
...
YouTubeToHtml5.prototype.loadSingle = function(a) {
    ...
    if (a.getAttribute(d)) {
        ...
        this.doAction("api.before", a),
        this.fetch(f).then(function(c) {
            if (c) {
                ...
                if (d && Array.isArray(d)) {
                    ...
                    h && "url"in h && h.url && (m.src = h.url),
                    h.type && "unknown" !== h.type && h.mime && "unknown" !== h.mime && (m.type = "".concat(h.type, "/").concat(h.mime)),
                    m.src = b.applyFilters("video.source", m.src, h, a, j, d),
                    m.src && "function" == typeof m.src.toString && m.src.toString().length ? (a.src = m.src,
